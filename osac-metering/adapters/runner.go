...
func NewRunner(adapter ProviderAdapter, cfg RunnerConfig, logger logr.Logger) *Runner {
	...
	return &Runner{
		...
		metrics: newAdapterMetrics(adapter.Name()),
...
func (r *Runner) processMessage(
	...
	if result.Err != nil {
		...
		// Track the offset even for failed events so the consumer does not
		// redeliver the same poison message on every restart.
		r.trackOffset(msg)
		return
	}
