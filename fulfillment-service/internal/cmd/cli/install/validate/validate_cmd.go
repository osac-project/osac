...
// bare `osac install validate` gates on everything OSAC might need. The
// Helm pre-install hook always passes --services explicitly, matching the
// services actually enabled in that install's values, so it never
// false-fails on an Operator a narrower install never needed.
...
func (r *runnerContext) run(cmd *cobra.Command, _ []string) error {
	...
	if r.args.json {
		...
		// Never paged, unlike `osac install discover`: validate is meant as
		// a scripted/CI gate (see longHelp) and a human running it
		// interactively wants a quick pass/fail, not a blocking pager.
		console.Infof(ctx, "Validating Hub cluster prerequisites...\n\n")
		console.Infof(ctx, "%s", render.Report(results, console.ColorEnabled(), render.Width(console.Stdout())))
	}

	if install.AnyRequiredFailed(results) {
		console.Errorf(ctx, "\nOne or more required prerequisites are not met; this Hub cluster is not ready to install OSAC.\n")
		return exit.Error(1)
	}
...
const shortHelp = `Fail if the Hub cluster is not ready to install OSAC`

const longHelp = `
Runs OSAC's prerequisite checks against the target Hub cluster and exits with
a non-zero status if any required prerequisite is not met, making it usable
as a scripted or CI gate immediately before {{ bt }}helm install{{ bt }}.

A failed {{ bt }}Warning{{ bt }}-severity check is still reported, but does not
...
Cluster state can drift between this command reporting success and
{{ bt }}helm install{{ bt }} actually running; run this immediately before
installing, not as a one-time check.
`
...
_[SERVICE...]{{ bt }},{{ bt }}...{{ bt }}]_ - Which OSAC services to require
prerequisites for: {{ bt }}vmaas{{ bt }}, {{ bt }}caas{{ bt }}, {{ bt }}bmaas{{ bt }},
{{ bt }}maas{{ bt }}, {{ bt }}metering{{ bt }}. Defaults to every service, so a bare
{{ bt }}osac install validate{{ bt }} gates on everything OSAC might need; narrow
it to match {{ bt }}global.services.*{{ bt }} in your {{ bt }}my-values.yaml{{ bt }} to
avoid failing on Operators an install that only enables some services doesn't
need.
`

const metal3FlagHelp = `
_[BOOLEAN]_ - Also require Metal3 bare-metal prerequisites: the BareMetalHost
...
_NAMESPACE/NAME[:KEY,...]_ - Also require that the named Secret exists (and,
if given, that it has every listed key). Repeatable. Use this to require
...
consumption. Nothing else is printed to stdout: no intro line, no color.
The exit code and the "not ready" message on stderr are unaffected.
`
