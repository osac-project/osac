...
var _ = DescribeMigration("Add name uniqueness and immutability", func() {
	...
	It("Enforces global uniqueness for roles", func(ctx context.Context) {
		createTenantAndProject(ctx, "other-tenant")
	...
	It("Enforces tenant-scoped uniqueness for users", func(ctx context.Context) {
		...
		// Same name after soft-delete: allowed (partial index excludes deleted rows)
		_, err = conn.Exec(ctx,
			`update users set deletion_timestamp = now() where id = $1`,
			"u1",
		)
		Expect(err).ToNot(HaveOccurred())
	...
	It("Replaces partial index on cluster_versions", func(ctx context.Context) {
		...
		// Before migration 100, reusing the name of a soft-deleted row is allowed
		...
		// Migration 100 resolves the collision (renames the soft-deleted row)
		// and creates a full unique index covering all rows.
		err = tool.Migrate(ctx, 100)
		Expect(err).ToNot(HaveOccurred())
	...
	It("Does not modify bare_metal_instance_types", func(ctx context.Context) {
		createTenantAndProject(ctx, "t1")

		err := tool.Migrate(ctx, 100)
		Expect(err).ToNot(HaveOccurred())

		// bare_metal_instance_types already had UNIQUE(name) globally and 'name' in its trigger.
		...
		// Same name, different tenant: rejected (global uniqueness preserved)
		_, err = conn.Exec(ctx,
			`insert into bare_metal_instance_types (id, name, tenant, data)
			 values ($1, $2, $3, '{}')`,
			"bmit2", "large-gpu", "shared",
		)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("unique"))

		// Name immutability still enforced
		_, err = conn.Exec(ctx,
			`update bare_metal_instance_types set name = $1 where id = $2`,
			"renamed", "bmit1",
		)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("immutable"))
	...
	It("Does not modify identity_providers", func(ctx context.Context) {
		...
		// Verify both still work as before.
		idpData := func(clientID string) string {
			return fmt.Sprintf(
				`{"spec":{"type":"IDENTITY_PROVIDER_TYPE_OPEN_ID_CONNECT","open_id_connect":{"issuer_url":"https://example.com","client_id":"%s","client_secret":"test-fixture-value"}},"status":{}}`,
				clientID,
			)
		}
		_, err = conn.Exec(ctx,
	...
	DescribeTable(
		...
		Entry(nil, "bare_metal_instance_catalog_items"),
		Entry(nil, "bare_metal_instance_templates"),
		Entry(nil, "bare_metal_instances"),
		Entry(nil, "clusters"),
		...
		Entry(nil, "host_types"),
		Entry(nil, "hubs"),
		Entry(nil, "instance_types"),
		Entry(nil, "nat_gateways"),
		Entry(nil, "network_classes"),
		Entry(nil, "objects"),
		Entry(nil, "project_memberships"),
		Entry(nil, "roles"),
