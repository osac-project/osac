...
exit-criterion (e) (OSAC-4008): these two files were trimmed down to only
the content with no equivalent in any single component's own `AGENTS.md`,
and relocated here, co-located with the code they analyze. The other five
`reference/*.md` siblings (`CONCERNS.md`, `INTEGRATIONS.md`, `STACK.md`,
`STRUCTURE.md`, `TESTING.md`) had zero live consumers and will be deleted
(rather than relocated) as part of the companion `osac-workspace` cleanup
PR referenced above.
