...
var _ = Describe("adapterMetrics", func() {
	...
	BeforeEach(func() {
		m = newAdapterMetrics("test-provider")
	})
	...
	Describe("gauge operations", func() {
		It("sets consumer lag", func() {
			m.consumerLag.WithLabelValues("test-provider", "topic-1").Set(42)
			val := testutil.ToFloat64(m.consumerLag.WithLabelValues("test-provider", "topic-1"))
